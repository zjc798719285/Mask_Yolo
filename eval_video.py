...
unet.eval()
unet.load_state_dict(th.load('.\checkpoint\\PersonMasker100.pt'))
...
while(True):
     ...
     t1 = time.time()
     mask_256, bbox_256, _, conf_256, get_box = unet(th.cuda.FloatTensor(frame_512))
     conf_256 = conf(conf_256)
     t2 = time.time()
     box_512 = mask_nms(mask=mask_256, box=get_box, conf=conf_256, mask_thresh=0.6, conf_thresh=0.4, roi_thresh=0.6)
     t3  =time.time()
