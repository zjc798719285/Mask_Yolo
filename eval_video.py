...
unet.eval()
unet.load_state_dict(th.load('E:\Person_detection\Mask_Yolo\checkpoint\\PersonMaskerUnitBox_27.pt'))
...
while(True):
     ...
     t1 = time.time()
     mask_128, bbox_128 = unet(th.cuda.FloatTensor(frame_512))
     ...
     mask = cv2.resize(np.transpose(mask_128.detach().cpu().numpy()[0, :, :, :], [1, 2, 0]), (frame.shape[1] // 2, frame.shape[0] // 2))
